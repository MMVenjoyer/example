...
async def get_data_by_article(product_article: int):
	...
	async with aiohttp.ClientSession() as session:
		async with session.get(f'https://card.wb.ru/cards/v1/detail?appType=1&curr=rub&dest=-1257786&spp=30&nm={product_article}') as result:
			...
			if result: # Проверяем успешен ли запрос
...
async def get_generated_answer(text_to_generate: tuple, status: bool):
...
async def get_start(message: types.Message):
	await bot.send_message(message.from_user.id, 'Старт', reply_markup=kb.kb_menu)
...
async def get_main_menu(message: types.Message):
	if message.text == 'Получить информацию по товару✳️': # Получаем инфу по артиклю
		...
		async def get_product_info(message: types.Message):
			result = await asyncio.create_task(get_data_by_article(int(message.text)))
			if result: # Проверяем успешен ли запрос
				try:
